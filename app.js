...
let sortedByBirthYear = inventors.sort((inventorA, inventorB) => {
    return inventorA.year - inventorB.year; // if I want descend just replace a with b
});
...
let firstLast = people.map(peron => {
    let firts = peron.split(", ")
    return firts[1] + " " + firts[0]
});
